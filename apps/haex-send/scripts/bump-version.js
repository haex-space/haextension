...
const EXTENSION_SLUG = 'haex-send';
...
async function uploadBundle(apiKey, version, bundlePath) {
  ...
  formData.append('bundle', new Blob([bundleData], { type: 'application/octet-stream' }), `haex-send-${version}.xt`);
...
if (shouldTag) {
  const tagName = `haex-send-v${newVersion}`;
  console.log(`\nCreating release tag: ${tagName}`);

  exec(`git add apps/haex-send/package.json`);
  if (existsSync(manifestPath)) {
    exec(`git add apps/haex-send/haextension/manifest.json`);
  }
  exec(`git commit -m "chore(haex-send): bump version to ${newVersion}"`);
...
if (shouldUpload) {
  console.log('\n📦 Building extension...');
  exec(`pnpm --filter haex-send build:release`);

  const bundlePath = join(rootDir, `haex-send-${newVersion}.xt`);
