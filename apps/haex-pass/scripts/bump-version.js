...
const EXTENSION_SLUG = 'haex-pass';
...
async function uploadBundle(apiKey, version, bundlePath) {
  ...
  formData.append('bundle', new Blob([bundleData], { type: 'application/octet-stream' }), `haex-pass-${version}.xt`);
...
if (shouldTag) {
  const tagName = `haex-pass-v${newVersion}`;
  console.log(`\nCreating release tag: ${tagName}`);

  exec(`git add apps/haex-pass/package.json`);
  if (existsSync(manifestPath)) {
    exec(`git add apps/haex-pass/haextension/manifest.json`);
  }
  exec(`git commit -m "chore(haex-pass): bump version to ${newVersion}"`);
...
if (shouldUpload) {
  console.log('\n📦 Building extension...');
  exec(`pnpm --filter haex-pass build:release`);

  const bundlePath = join(rootDir, `haex-pass-${newVersion}.xt`);
